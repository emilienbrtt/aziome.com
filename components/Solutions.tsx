...
/* ===== Réglages =====
   Desktop inchangé. Mobile uniquement ci-dessous.
*/
const TUNE_DESKTOP = {
...
const TUNE_MOBILE = {
  // scale = taille (inchangée), offsetY = vers le BAS (+), pt = marge haut, pb = marge bas (gradient)
  center: { scale: 1.35, offsetY: 28, pt: 12, pb: 92 }, // ↓ perso plus bas, mais même taille
};
    ...
    return (
      <div className={roleClass(role)} tabIndex={-1}>
        ...
        <div
          ...
          style={{ background: 'radial-gradient(120% 140% at 50% 0%, rgba(212,175,55,0.22), rgba(246,231,178,0.10), rgba(0,0,0,0) 70%)' }}
        />
        ...
        <div className="relative bg-black h-[340px] sm:h-[380px] lg:h-[420px]">
          <div className="absolute inset-0" style={{ paddingTop: cfg.pt ?? 0, paddingBottom: cfg.pb }}>
            <Image
              ...
              style={{ transform: `translateY(${cfg.offsetY}px) scale(${cfg.scale})`, objectPosition: 'center bottom' }}
            />
          </div>
          <div className="absolute inset-x-0 bottom-0 bg-gradient-to-b from-transparent to-black/70" aria-hidden style={{ height: cfg.pb }} />
        </div>
        ...
        <div className="p-5">
          ...
          <Link href={`/agents/${data.slug}`} className="mt-3 inline-block text-sm text-[color:var(--gold-1)]">Voir les détails →</Link>
        </div>
    ...
    <section id="solutions" className="max-w-6xl mx-auto px-6 py-20">
      ...
      <div className="hidden md:flex items-stretch justify-center gap-5 overflow-visible">
        <div className="relative w-[42%] md:w-[34%] lg:w-[30%] xl:w-[28%]">
          <button onClick={goPrev} className="hidden sm:flex items-center justify-center absolute left-1 top-1/2 -translate-y-1/2 z-30 h-12 w-12 rounded-full ring-1 ring-white/15 bg-white/5 hover:ring-[rgba(212,175,55,0.55)] hover:bg-white/10 hover:shadow-[0_0_70px_rgba(212,175,55,0.35)] transition" aria-label="Précédent">
        ...
        <div className="relative w-[42%] md:w-[34%] lg:w-[30%] xl:w-[28%]">
          <button onClick={goNext} className="hidden sm:flex items-center justify-center absolute right-1 top-1/2 -translate-y-1/2 z-30 h-12 w-12 rounded-full ring-1 ring-white/15 bg-white/5 hover:ring-[rgba(212,175,55,0.55)] hover:bg-white/10 hover:shadow-[0_0_70px_rgba(212,175,55,0.35)] transition" aria-label="Suivant">
      ...
      <div className="md:hidden relative px-2" onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
        ...
        {/* Vignettes : coins arrondis + contour blanc, PAS de bleu iOS */}
        <div className="mt-4">
              ...
              return (
                <li key={c.slug} className="shrink-0">
                  <button
                    ...
                    className={[
                      'mini-thumb h-12 w-12 rounded-2xl flex items-center justify-center',
                      'ring-1 outline-none focus:outline-none focus-visible:outline-none', // supprime halo bleu
                      active
                        ? 'ring-[rgba(212,175,55,0.70)] shadow-[0_0_22px_rgba(212,175,55,0.28)] bg-black/30'
                        : 'ring-white/22 hover:ring-white/32 bg-black/10'
                    ].join(' ')}
                    aria-label={c.name}
                    style={{ WebkitTapHighlightColor: 'transparent' }}
                  >
      ...
      {/* style helper pour masquer le scroll-bar mobile si la classe n'existe pas déjà */}
      <style jsx>{`
